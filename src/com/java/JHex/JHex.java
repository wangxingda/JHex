...
public final class JHex {
	...
	public static byte[] toByteArray(int[] numbers, int from, int to) {
		...
		for (int i = 0; i < len; i++) {
			for (int j = 0; j < 4; j++) {
				bytes[j + i * 4] = (byte) (numbers[i + from] >>> (24 - j * 8));
			}
		}
		return bytes;
	}
	...
	public static byte[] toByteArray(float[] numbers, int from, int to) {
		...
		for (int i = 0; i < len; i++) {
			for (int j = 0; j < 4; j++) {
				bytes[j + i * 4] = (byte) (Float.floatToIntBits(numbers[i + from]) >>> (24 - j * 8));
			}
		}
		return bytes;
	}
	...
	public static byte[] toByteArray(double[] numbers, int from, int to) {
		...
		for (int i = 0; i < len; i++) {
			for (int j = 0; j < 8; j++) {
				bytes[j + i * 8] = (byte) (Double.doubleToLongBits(numbers[i + from]) >>> (56 - j * 8));
			}
		}
		return bytes;
	}
	...
	public static float[] toFloatArray(byte[] arr, int from, int to) {
		...
		int intBits = 0;
		for (int i = 0; i < result.length; i++) {
			intBits = toInt(arr, from + i * 4, from + (i + 1) * 4);
